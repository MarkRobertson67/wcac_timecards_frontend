...
//         console.log(`Cannot update entry for date ${entry.work_date} as it is already submitted.`);
//         alert(`Cannot modify the entry for date ${entry.work_date} because it has already been submitted.`);
...
function ActiveTimeCard({ setIsNewTimeCardCreated }) {
  ...
  const getPreviousMonday = (date) => {
    ...
    if (day === 1) {
      ...
      return utcDate.add(1, 'days');
    } else {
      return utcDate.startOf('week').add(1, 'days');
    }
  };

  const getEndDate = (startDate) => {
    return moment(startDate).add(13, 'days');
  };
  ...
  const fetchTimeCardData = useCallback(async (startDate) => {
    try {
      ...
      const missingDates = allWeekdays.map(date => date.format('YYYY-MM-DD')).filter(date => !fetchedEntriesMap.has(date));
      console.log("Missing Dates:", missingDates);
  ...
  //   console.log("Starting submission process..."); // Log when starting
  //   console.log("Entries being submitted:", twoWeekPeriod); // Log entries before any checks
  ...
  //     await Promise.all(
  //       twoWeekPeriod.map(async (entry) => {
  //         const entryDate = entry.work_date || 'No date found';
  //         console.log(`Processing entry for date: ${entryDate}, ID: ${entry.id ? entry.id : 'No ID assigned yet'}`);
  ...
  //           } catch (error) {
  //             console.error(`Error during PUT operation for date ${entryDate}:`, error);
  //             failedSubmissions.push(entryDate);
  //           }
  //         }
  //       })
  //     );
  ...
  const handleSubmit = async () => {
    ...
    // Check if all entries are already submitted
    if (twoWeekPeriod.every(entry => entry.status === 'submitted')) {
        alert("All entries are already submitted.");
        return; // Prevent resubmission
    }

    // Define required fields and check for incomplete entries
    const requiredFields = ['startTime', 'lunchStart', 'lunchEnd', 'endTime'];
    const incompleteEntries = twoWeekPeriod.filter(entry =>
        requiredFields.some(field => !entry[field] || entry[field].trim() === '')
    );
    ...
    if (incompleteEntries.length > 0) {
        const confirmation = window.confirm(
            `There are ${incompleteEntries.length} entries with missing required fields. Click 'Cancel' to create them first, or 'OK' to proceed.`
        );
    ...
    try {
        const failedSubmissions = await Promise.all(
            twoWeekPeriod.map(async (entry) => {
                if (!entry.id) return; // Skip entries without an ID

                const url = `${API}/timecards/${entry.id}`;
                const requestPayload = {
                    status: 'submitted',
                    work_date: entry.work_date // No need to reformat the date
                };
                ...
                try {
                    ...
                    if (!response.ok) {
                        const errorMessage = await response.text();
                        console.error(`Failed to submit entry for ID ${entry.id}: ${errorMessage}`);
                        return entry.work_date; // Return the date for failed submissions
                    }
                    console.log(`Successfully updated entry with ID: ${entry.id}`);
                } catch (error) {
                    console.error(`Error during PUT operation for entry ID ${entry.id}:`, error);
                    return entry.work_date; // Return the date for failed submissions
                }
            })
        );
        ...
        if (failedSubmissions.filter(date => date).length === 0) {
            console.log('All submissions succeeded. Triggering confetti.');
            setShowConfetti(true);
            setIsSubmitted(true);
            ...
            const failedDatesFormatted = failedSubmissions.filter(Boolean).join(', ');
            alert(`Timecard submitted with errors. Failed to submit entries for the following dates:\n${failedDatesFormatted}`);
            console.log(`Partial failures for dates: ${failedDatesFormatted}`);
        }
